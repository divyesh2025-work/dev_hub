...
# Don't use cdef public - use plain Python functions that we'll expose via C wrapper

def _py_strategy_get_type_id():
    return CONREV_IOC_PYTHON_TYPE_ID

def _py_strategy_create():
    global _current_strategy
    _current_strategy = ConRevIOCStrategy()
    Py_INCREF(_current_strategy)
    return <object>_current_strategy

def _py_strategy_destroy_all():
    global _current_strategy
    _current_strategy = None

# Now create C wrappers in a separate .cpp file or via extern block
cdef extern from *:
    """
    #include <Python.h>
    ...
    void* strategy_create(StrategyFnTable* tbl) {
        ...
        PyObject* module = PyImport_ImportModule("conrev_ioc_python");
        if (!module) {
            PyGILState_Release(gstate);
            return NULL;
        }
        
        PyObject* create_func = PyObject_GetAttrString(module, "_py_strategy_create");
        PyObject* strategy_obj = PyObject_CallObject(create_func, NULL);
        ...
        // Fill function table
        extern int32_t c_on_add(void*, PlatformContext*, const PlatformAPI*, uint32_t, 
                                const uint8_t*, uint32_t, uint8_t*, uint32_t*);
        extern int32_t c_on_edit(void*, PlatformContext*, uint32_t, const uint8_t*, 
                                 uint32_t, uint8_t*, uint32_t*);
        extern int32_t c_on_run(void*, PlatformContext*, uint32_t, uint8_t*, uint32_t*);
        extern int32_t c_on_stop(void*, PlatformContext*, uint32_t, uint8_t*, uint32_t*);
        extern int32_t c_on_remove(void*, PlatformContext*, uint32_t, uint8_t*, uint32_t*);
        extern int32_t c_on_query(void*, PlatformContext*, uint32_t, uint8_t*, uint32_t*);
        extern void c_on_market_event(void*, PlatformContext*, uint32_t, const MarketEvent*);
        extern void c_on_order_update(void*, PlatformContext*, uint32_t, const OrderUpdate*);
    ...
    void strategy_destroy_all(void) {
        ...
        PyObject* module = PyImport_ImportModule("conrev_ioc_python");
        if (module) {
            PyObject* destroy_func = PyObject_GetAttrString(module, "_py_strategy_destroy_all");
            if (destroy_func) {
                PyObject_CallObject(destroy_func, NULL);
                Py_DECREF(destroy_func);
            }
            Py_DECREF(module);
        }
